...
def researcher_node(state: IdeaRadarState):
    ...
    try:
        ...
        if not tavily_key or tavily_key == "your_tavily_api_key_here":
            raise ValueError("Tavily API key is missing or invalid in .env")
        ...
        if macro_answer:
            ...
            new_raw_sources.append({
                ...
                "author": "Tavily AI",
                "url": "",
                "story_title": f"Expert Market Intelligence: {topic}",
                "date": "Recent"
            })
...
def analyst_node(state: IdeaRadarState):
    ...
    ollama_key = os.environ.get("OLLAMA_API_KEY")
    headers = {"Authorization": f"Bearer {ollama_key}"} if ollama_key else {}
    llm = ChatOllama(model="nemotron-3-nano:30b-cloud", base_url=ollama_url, headers=headers, format="json", temperature=0)
...
def economist_node(state: IdeaRadarState):
    ...
    queries = [
        ...
        f"{target_cust} market valuation report {location}",
        f"industry growth rate for {problem_name} {location}"
    ]
    ...
    prompt = PromptTemplate(
        ...
        Return ONLY valid JSON.
        """,
        input_variables=["problem_name", "target_customer", "search_data", "location"]
    )
