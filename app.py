...
from src.database import IdeaRadarDB

# Initialize Database
db = IdeaRadarDB()

st.set_page_config(page_title="IdeaRadar", page_icon="📡", layout="wide", initial_sidebar_state="expanded")

# Premium CSS for Radar Aesthetic
st.markdown("""
<style>
    ...
    /* Glass Cards */
    [data-testid="stVerticalBlock"] > div > div > div > div.problem-card {
    ...
    .stTabs [aria-selected="true"] {
        background-color: rgba(96, 239, 255, 0.1) !important;
        border-bottom: 3px solid #60efff !important;
...
def check_password():
    ...
    def password_entered():
        ...
        if st.session_state["password"] == os.environ.get("APP_PASSWORD", "ridarai2024"):
    ...
    if "password_correct" not in st.session_state:
        ...
        st.markdown("""
            <div style='text-align: center; padding: 50px;'>
                <h1 style='font-size: 3rem; background: linear-gradient(45deg, #00ff87, #60efff); -webkit-background-clip: text; -webkit-text-fill-color: transparent;'>
                    📡 IdeaRadar Command Center
                </h1>
                <p style='color: #888; font-size: 1.2rem;'>Restricted Access Intelligence Terminal</p>
            </div>
        """, unsafe_allow_html=True)
        ...
        with col:
            ...
            st.caption("IdeaRadar v1.0 | Enterprise Edition")
        return False
    else:
...
st.markdown("<h1 class='radar-title'>📡 IdeaRadar</h1>", unsafe_allow_html=True)
st.markdown("<p class='radar-subtitle'>Multi-Agent System: Discover Market Gaps and Generate Blueprints</p>", unsafe_allow_html=True)
...
def process_stream(stream_generator, status_container, log_container=None):
    progress_bar = st.progress(0, text="📡 Initializing Multi-Agent Radar...")
    agent_count = 0
...
with st.sidebar:
    ...
    st.markdown("---")
    if st.button("➕ Start New Radar", use_container_width=True):
...
if st.session_state.app_stage == "processing1":
    with st.status("📡 IdeaRadar Intelligence Pipeline Active...", expanded=True) as status:
    ...
    with tab1:
        ...
        if voc:
            for item in voc:
                ...
                st.markdown(f"""
                <div style='background:#0f172a;border-left:4px solid #60efff;padding:14px 18px;border-radius:8px;margin-bottom:12px'>
                    <span style='color:#e2e8f0;font-style:italic'>"{item.get('quote','')}"</span><br>
                    <span style='font-size:0.8rem;'>— {attribution}</span>
                </div>
                """, unsafe_allow_html=True)
        else:
